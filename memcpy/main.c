...
static uint32_t MemPerThread; /* in GB/therad */
static int quit = 0;
static pthread_t thread[16];
...
static void __attribute__ ((noinline)) v1_CopyCompare(const int offsetIndex)
{
        ...
        assert(memcmp(dst + offIndex * MemPerThread, src + offIndex * MemPerThread, MemPerThread) == 0);
}
...
static void __attribute__ ((noinline)) v5_CopyCompare(const int offsetIndex)
{
        ...
        for (i = 64; i < MemPerThread - 64; i += 64)
        {
...
static void __attribute__ ((noinline)) v6_CopyCompare(const int offsetIndex)
{
        ...
        for (i = 128, j = 0; i < MemPerThread - 128; i += 128, j += 128)
        {
...
{
        const int threadIndex = (int *)userData;
...
void help_usage(void)
{
        ...
        printf("------------------------------------------------------\n");
        printf(" Usage: \n");
        printf(" - ./build/test   ==> runs with default 8 threads \n");
        printf(" - ./build/test 1 ==> runs with default 1 threads \n");
        printf("------------------------------------------------------\n");
        printf("\n\n");
        return;
}
...
{
        if (argc > 2) {
                printf("ERR: unexpected arguments {%d}\n", argc);
        ...
        if (argc == 2) {
                ...
                numThreads = (numThreads > 16) ? 16 :  (numThreads < 1) ? 1 : numThreads;
        }

        MemPerThread = (numMemGb/numThreads) * (1UL << 31); /* in GB/therad */
        ...
        printf("\033[%d;1H speedup %%: %f, %f, %f, %f, %f, %f, %f, %f",
                ...
                ((double)total[0] / (double)total[6]),
                ((double)total[0] / (double)total[7]) );
        printf("\033[%d;1H ---------------------------------", i + 5);
